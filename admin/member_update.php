...
if ($no <= 0 || $id === "" || $name === "" || $pass === "" || $age <= 0 || $email === "" || $phone === "" || $role === "")
    die("잘못된 요청");
...
$strSQL = "UPDATE member set id='{$id_esc}', m_name='{$name_esc}', pass='{$pass_esc}', age='{$age}', email='{$email_esc}', phone='{$phone_esc}', role='{$role_esc}' WHERE no={$no}";
echo $strSQL;
